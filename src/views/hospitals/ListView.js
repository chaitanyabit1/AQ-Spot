...
import defaultImg from "../../assets/img/default-hospital.png";
import {
    MODULE_ACTION,
    MODULE_ACTION_SEP,
    extractModuleData,
...
const ListView = (props) => {
  ...
  const handleActionSelect = (val) => {
    ...
    switch (actionData[0]) {
      ...
      case MODULE_ACTION.EDIT:
        page = (RoutePaths.HOSPITALS_EDIT).replace(":id", actionData[1]);
        history.push(page);
        break;
  ...
  return (
    <div className="row hospitals_view">
              ...
              <div className="card-actions-float">
                <Dropdown id={Math.random()} onSelect={handleActionSelect} className="data-action">
                  ...
                  <Dropdown.Menu>
                    ...
                    >View</Dropdown.Item>
                    {(getUserAccesses().includes(USER_RESTRICT_ROUTE.HOSPITAL_EDIT) || [USER_TYPE.SUPER_ADMIN].includes(getUserDetails().user_type)) &&
                      <><Dropdown.Item 
                        eventKey={
                          MODULE_ACTION.EDIT +
                          MODULE_ACTION_SEP +
