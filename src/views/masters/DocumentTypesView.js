...
const DocumentTypesView = (props) => {
  const addButtonText = "Add New Document Type";
  ...
  // Fetch data
  useEffect(() => {
    const q = (currentPage) ? "&page="+Number(currentPage) : "";
    listAll("document_types", q).then((res) => {
  ...
  // On record delete
  const onRowDelete = () => {
    deletePage("document_types", deleteRowId).then((res) => {
...
const AddForm = (props) => {
  ...
  const onSubmit = (data) => {
    ...
    create("document_types", saveData).then((res) => {
...
const EditForm = (props) => {
  ...
  useEffect(() => {
    ...
    viewPage("document_types", rowId).then((res) => {
      setRowInfo(res.data);
  ...
  const onSubmit = (data) => {
    ...
    update("document_types", saveData, rowId).then((res) => {
      handleCloseAndReload();
    }).catch((err) => {
      NotificationManager.error(COMMON_MESSAGES.SOMETHING_WRONG, 'Error', 5000);
    });
