...
const InvestigationTypesView = (props) => {
  const addButtonText = "Add New Investigation Type";
  ...
  useEffect(() => {
    ...
    listAll("investigation_types", q).then((res) => {
  ...
  // On record delete
  const onRowDelete = () => {
    deletePage("investigation_types", deleteRowId).then((res) => {
...
const AddForm = (props) => {
  ...
  const onSubmit = (data) => {
    ...
    create("investigation_types", saveData).then((res) => {
  ...
  return (
    ...
    >
      <Modal.Header closeButton>
        <Modal.Title>Add Investigation Type</Modal.Title>
      </Modal.Header>
      <Modal.Body>
...
const EditForm = (props) => {
  ...
  useEffect(() => {
    ...
    viewPage("investigation_types", rowId).then((res) => {
      setRowInfo(res.data);
  ...
  const onSubmit = (data) => {
    ...
    update("investigation_types", saveData, rowId).then((res) => {
      handleCloseAndReload();
    }).catch((err) => {
      NotificationManager.error(COMMON_MESSAGES.SOMETHING_WRONG, 'Error', 5000);
    });
  ...
  return (
    ...
    >
      <Modal.Header closeButton>
        <Modal.Title>Edit Investigation Type</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="form-group">
...
export default InvestigationTypesView;
