...
const PastHistoryParamsView = (props) => {
  const addButtonText = "Add Past History Params";
  ...
  useEffect(() => {
    ...
    listAll("past_history_params", q).then((res) => {
  ...
  // On record delete
  const onRowDelete = () => {
    deletePage("past_history_params", deleteRowId).then((res) => {
...
const AddForm = (props) => {
  ...
  useEffect(() => {
    listAll("past_history_types").then((res) => {
  ...
  const onSubmit = (data) => {
    ...
    create("past_history_params", saveData).then((res) => {
  ...
  return (
    ...
    >
      <Modal.Header closeButton>
        <Modal.Title>Add Past History Params</Modal.Title>
      </Modal.Header>
      <Modal.Body>
...
const EditForm = (props) => {
  ...
  useEffect(() => {
    ...
    viewPage("past_history_params", rowId).then((res) => {
      setRowInfo(res.data);
  ...
  const onSubmit = (data) => {
    ...
    update("past_history_params", saveData, rowId).then((res) => {
      handleCloseAndReload();
    }).catch((err) => {
      NotificationManager.error(COMMON_MESSAGES.SOMETHING_WRONG, 'Error', 5000);
    });
  ...
  return (
    ...
    >
      <Modal.Header closeButton>
        <Modal.Title>Edit Past History Params</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="form-group">
...
export default PastHistoryParamsView;
