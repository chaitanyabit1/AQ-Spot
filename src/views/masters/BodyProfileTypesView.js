...
const BodyProfileTypesView = (props) => {
  const addButtonText = "Add New Body Profile Type";
  ...
  useEffect(() => {
    ...
    listAll("body_profile_types", q).then((res) => {
  ...
  // On record delete
  const onRowDelete = () => {
    deletePage("body_profile_types", deleteRowId).then((res) => {
...
const AddForm = (props) => {
  ...
  const onSubmit = (data) => {
    ...
    create("body_profile_types", saveData).then((res) => {
  ...
  return (
    ...
    >
      <Modal.Header closeButton>
        <Modal.Title>Add Body Profile Type</Modal.Title>
      </Modal.Header>
      <Modal.Body>
...
const EditForm = (props) => {
  ...
  useEffect(() => {
    ...
    viewPage("body_profile_types", rowId).then((res) => {
      setRowInfo(res.data);
  ...
  const onSubmit = (data) => {
    ...
    update("body_profile_types", saveData, rowId).then((res) => {
      handleCloseAndReload();
    }).catch((err) => {
      NotificationManager.error(COMMON_MESSAGES.SOMETHING_WRONG, 'Error', 5000);
    });
  ...
  return (
    ...
    >
      <Modal.Header closeButton>
        <Modal.Title>Edit Body Profile Type</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="form-group">
...
export default BodyProfileTypesView;
