...
const PastHistoryTypesView = (props) => {
  const addButtonText = "Add New Past History Type";
  ...
  useEffect(() => {
    ...
    listAll("past_history_types", q).then((res) => {
  ...
  // On record delete
  const onRowDelete = () => {
    deletePage("past_history_types", deleteRowId).then((res) => {
...
const AddForm = (props) => {
  ...
  const onSubmit = (data) => {
    ...
    create("past_history_types", saveData).then((res) => {
  ...
  return (
    ...
    >
      <Modal.Header closeButton>
        <Modal.Title>Add Past History Type</Modal.Title>
      </Modal.Header>
      <Modal.Body>
...
const EditForm = (props) => {
  ...
  useEffect(() => {
    ...
    viewPage("past_history_types", rowId).then((res) => {
      setRowInfo(res.data);
  ...
  const onSubmit = (data) => {
    ...
    update("past_history_types", saveData, rowId).then((res) => {
      handleCloseAndReload();
    }).catch((err) => {
      NotificationManager.error(COMMON_MESSAGES.SOMETHING_WRONG, 'Error', 5000);
    });
  ...
  return (
    ...
    >
      <Modal.Header closeButton>
        <Modal.Title>Edit Past History Type</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="form-group">
...
export default PastHistoryTypesView;
