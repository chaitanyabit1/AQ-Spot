...
const FollowupTypesView = (props) => {
  const addButtonText = "Add New Followup Type";
  ...
  useEffect(() => {
    ...
    listAll("followup_types", q).then((res) => {
  ...
  // On record delete
  const onRowDelete = () => {
    deletePage("followup_types", deleteRowId).then((res) => {
...
const AddForm = (props) => {
  ...
  const onSubmit = (data) => {
    ...
    create("followup_types", saveData).then((res) => {
  ...
  return (
    ...
    >
      <Modal.Header closeButton>
        <Modal.Title>Add Followup Type</Modal.Title>
      </Modal.Header>
      <Modal.Body>
...
const EditForm = (props) => {
  ...
  useEffect(() => {
    ...
    viewPage("followup_types", rowId).then((res) => {
      setRowInfo(res.data);
  ...
  const onSubmit = (data) => {
    ...
    update("followup_types", saveData, rowId).then((res) => {
      handleCloseAndReload();
    }).catch((err) => {
      NotificationManager.error(COMMON_MESSAGES.SOMETHING_WRONG, 'Error', 5000);
    });
  ...
  return (
    ...
    >
      <Modal.Header closeButton>
        <Modal.Title>Edit Followup Type</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="form-group">
...
export default FollowupTypesView;
