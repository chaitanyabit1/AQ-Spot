...
const UserTypeView = (props) => {
  const addButtonText = "Add New User Type";
  ...
  // On record delete
  const onRowDelete = () => {
    deletePage("user_types", deleteRowId).then((res) => {
...
const AddForm = (props) => {
  ...
  const onSubmit = (data) => {
    ...
    create("user_types", saveData).then((res) => {
    }).catch((err) => {
      NotificationManager.error(COMMON_MESSAGES.SOMETHING_WRONG, 'Error', 5000);
    });
...
const EditForm = (props) => {
  ...
  useEffect(() => {
    ...
    viewPage("user_types", rowId).then((res) => {
      setRowInfo(res.data);
  ...
  const onSubmit = (data) => {
    ...
    update("user_types", saveData, rowId).then((res) => {
      handleCloseAndReload();
    }).catch((err) => {
      NotificationManager.error(COMMON_MESSAGES.SOMETHING_WRONG, 'Error', 5000);
    });
