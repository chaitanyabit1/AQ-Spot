...
const MedicineTypesView = (props) => {
  const addButtonText = "Add New Medicine Type";
  ...
  useEffect(() => {
    ...
    listAll("medicine_types", q).then((res) => {
  ...
  // On record delete
  const onRowDelete = () => {
    deletePage("medicine_types", deleteRowId).then((res) => {
...
const AddForm = (props) => {
  ...
  const onSubmit = (data) => {
    ...
    create("medicine_types", saveData).then((res) => {
    }).catch((err) => {
      NotificationManager.error(COMMON_MESSAGES.SOMETHING_WRONG, 'Error', 5000);
    });
...
const EditForm = (props) => {
  ...
  useEffect(() => {
    ...
    viewPage("medicine_types", rowId).then((res) => {
      setRowInfo(res.data);
  ...
  const onSubmit = (data) => {
    ...
    update("medicine_types", saveData, rowId).then((res) => {
      handleCloseAndReload();
    }).catch((err) => {
      NotificationManager.error(COMMON_MESSAGES.SOMETHING_WRONG, 'Error', 5000);
    });
  ...
  return (
    ...
    >
      <Modal.Header closeButton>
        <Modal.Title>Edit Medicine Type</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="form-group">
...
export default MedicineTypesView;
